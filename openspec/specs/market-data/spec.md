...
The system SHALL calculate RSI and SMA technical indicators on received candle data.

#### Scenario: Indicators calculated on candle close

- **WHEN** a candle closes (`CandleClosedNotification` published)
- **THEN** `RsiCandleClosedHandler` calculates RSI and SMA values for configured lengths
- **AND** publishes `SmasAndRsisCalculatedEvent` with the results
