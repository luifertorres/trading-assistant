...
TradingPlatform MarketData SHALL persist OHLCV bars in a **canonical single-table candles store** keyed by `(instrument_id, timeframe_id, open_time_ms)`, accessed through `SeriesDescriptor(InstrumentId, TimeFrameCode)`.
