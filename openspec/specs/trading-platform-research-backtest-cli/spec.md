...
TradingPlatform Research SHALL expose a **CLI delivery path** to run simulations against **persisted MarketData** `Day1` candles (from `backfill-1d` or equivalent load), without seeding synthetic bars or calling Binance REST during the backtest command.
